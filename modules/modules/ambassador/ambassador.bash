...
function __executor() {
  ...
  sudo docker build \
    --build-arg IMAGE_VERSION="${__aes_app_version}" \
    --build-arg FILENAME_ROOT_CA="$(getBaseFQDN).ca.crt" \
    -t docker.io/datawire/aes:${__aes_app_version} \
    -f "${RDBOX_WORKDIR_OF_SCRIPTS_BASE}/modules/modules/ambassador/subs/docker/Dockerfile" \
    "$(getDirNameFor outputs)"/ca
  sudo kind load docker-image docker.io/datawire/aes:${__aes_app_version} --name "$(getClusterName)"
  ### 2. helm
  ###
  echo ""
